 ...
 * @author Home
 */
public class VentaResumenController extends AbstractDialogController<Venta>
        implements VentaResumenService {
    ...
    public void createResumen(Date del, Date al, Cocina cocina) {
        ...
        LocalDate fechaDel = LocalDate.of(del.getYear() + 1900, del.getMonth() + 1, del.getDate());
        LocalDate fechaAl = LocalDate.of(al.getYear() + 1900, al.getMonth() + 1, al.getDate());

        while (fechaDel.isBefore(fechaAl)) {
            List<Venta> ventas = VentaDAO.getInstance().find(Date.from(fechaDel.atStartOfDay(ZoneId.systemDefault()).toInstant()));
            for (Venta ve : ventas) {
                if (ve != null) {
                    ...
                    if (initDateNotSet) {
                        v.setFecha(new Date(fechaDel.toEpochDay()));
