...
public class AreaVentaController extends OldAbstractListController<Area> implements AreaVentaService {
    ...
    @Override
    public void createMesa(Area a, String id) {
        if (a == null) {
            throw new IllegalArgumentException("Debe seleccionar un area");
        }
        if (id != null) {
            if (id.isEmpty()) {
                throw new IllegalArgumentException("El numero de mesa no puede ser vacio");
            } else {
                try {
    ...
    public void removeMesa(Mesa selectedValue) {
        if (selectedValue == null) {
            throw new IllegalArgumentException("Debe seleccionar una seccion");
        }
